...
function createNeptune() {
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        if (y < -1.5 && y > -2.5 && x > 0 && Math.abs(z) < 2.0) {
            if (Math.random() > 0.6) isStorm = true;
        }
...
function createTriton() {
    ...
    for(let i=0; i<tParticles; i++) {
        ...
        c.multiplyScalar(0.8 + Math.random() * 0.2);
    ...
    tritonData = {
        mesh: tBody,
        angle: 0,
        speed: -0.004, // [TUNE] 降速 50% (原 -0.008)
        radius: tOrbitRadius
    };
}
...
function createMinorMoons() {
    // [TUNE] 降速 50%
    const moonsConfig = [
        { name: "Galatea", radius: 5.5, speed: 0.015, size: 0.05, color: 0x6677aa }, // 原 0.03
        { name: "Larissa", radius: 5.75, speed: 0.0125, size: 0.06, color: 0x556699 }, // 原 0.025
        { name: "Proteus", radius: 6.0, speed: 0.01, size: 0.08, color: 0x6677aa }, // 原 0.02
        { name: "Nereid", radius: 24.0, speed: 0.001, size: 0.07, color: 0x8899cc, eccentric: true } // 原 0.002
    ];
